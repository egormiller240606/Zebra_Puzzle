...
- `Simulation.py` - основной файл симуляции
- `log_analyzer.py` - скрипт для анализа результатов симуляции
...
- `data/` - директория с данными
  ...
  - `simulation_log.csv` - лог событий симуляции (создается после запуска симуляции)
  - `cumulative_events_gpaph.png` - график нарастающего итога событий (создается анализатором)

## Требования

- Python 3.7+
- Стандартные библиотеки: `random`, `heapq`, `typing`
- Для анализа результатов: `numpy`, `matplotlib`
...
3. Вывод:
    ...
    - Финальные знания агентов (выводятся в консоль и сохраняются в лог)
...
- График нарастающего итога событий по типам (сохраняется в `data/cumulative_events_gpaph.png`)
...
- При успехе: обмен знаниями между присутствующими и генерация событий обмена домами (если условия выполнены)
...
### Обмен питомцами
- Происходит после прибытий агентов в дома (независимо от успеха встречи с владельцем)
